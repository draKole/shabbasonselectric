...
export function useBillsTotals(from: string, to: string, type?: "business" | "personal") {
  ...
  useEffect(() => {
    (async () => {
      ...
      let bq = supabase.from("bills").select("amount, due_date, paid, paid_on, bill_type, recurring").eq("recurring", false);
      if (type) bq = bq.eq("bill_type", type);
      const [{ data }, { data: oneTime }] = await Promise.all([q, bq]);
      const arr = [...((data || []) as any[]), ...((oneTime || []) as any[])];
